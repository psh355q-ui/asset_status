...
### [] Phase 2, T2.2: 계좌 카드 UI (FE) RED→GREEN
...
### [] Phase 2, T2.3: 거래 입력 API (BE) RED→GREEN
...
### [] Phase 2, T2.4: 거래 입력 폼 UI (FE) RED→GREEN
...
### [] Phase 3, T3.1: 보유 현황 계산 로직 (BE) RED→GREEN
...
- [ ] 테스트 먼저 작성됨
- [ ] 모든 계산 로직 테스트 통과
- [ ] 복잡한 케이스 (여러 매수/매도) 테스트됨
- [ ] 커버리지 >= 90% (핵심 로직)

---

### [] Phase 3, T3.2: yfinance 현재가 조회 (BE) RED→GREEN
